...
### 12. Proxy
  ...
  > *GoF:* 어떤 다른 객체로 접근하는 것을 통제하기 위해 그 객체의 대리자 혹은 자리채움 역할을 제공하는 패턴  
  > *위키:* 접근 조절, 비용 절감, 복잡도 감소를 위해 접근이 힘든 객체에 대한 대역을 제공

- 실제로 참조할 대상에 대한 참조자 관리(접근 제어, 생성, 삭제)
  - 일반적으로 서비스(실제 대상)의 수명 주기를 생성하고 관리

| 프록시 패턴 종류 | 설명 | 예 |
|---|---|---|
| 가상 프록시 | 객체 생성 등의 처리를 최적화(요청이 있을때만 용량이 큰 객체를 생성하도록 하는(=copy on write) 등) | 웹페이지 이미지 로딩 등 |
| 원격지 프록시 | 원격 서비스를 로컬에서 실행. 객체가 다른 주소 공간에 존재함을 숨김 |  |
| 보호용 프록시 | 액세스 제어. 요청한 대상이 권한이 있는지 확인 |  |
| 로깅 프록시 | 서비스 개체에 대한 요청 기록 |  |
| 캐싱 프록시 | 클라이언트의 요청 결과(일반적으로 용량이 큰)를 캐시하고, 캐시의 수명 주기를 관리해야하는 경우 |  |
| 스마트 참조 | 객체의 사용이 끝났을 때 해당 객체를 바로 free 시키기 위함 |  |

- 유의
  - Adapter: 인터페이스 변환 <-> Proxy: 대상과 동일한 인터페이스 제공
  - Decorator: 동적으로 하나 이상의 서비스를 추가하기 위함. client가 직접 decorator의 구성을 제어 <-> Proxy: 서비스 추가 불가. proxy 객체가 실 객체에 대한 접근 제어
